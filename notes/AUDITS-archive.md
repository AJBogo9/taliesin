# Taliesin audit records: archive (pre-2026-07-07)

Historical audit rounds, kept for reference. The current audit + its active detail live in [AUDITS.md](AUDITS.md); the active backlog is [backlog.md](backlog.md). Fully resolved findings here are summarized (full repro in git history).
...
## Findings detail (resolved; full repro in git)

The per-finding repro + root-cause for #1-#11 (large-output hang; in-place-build data loss; a bare-string nav item wiping the Quarto `website:` block; dead Cmd-K search under `mounts:`; deep-nesting stack-overflow abort; duplicate cross-ref labels; front-matter terminator false-positive; category-slug collision; listing id-target substring; comma-in-category filter break; cell-output / stale-reply robustness) plus the selected LOW list are in git history. The RESOLVED summary + Known residuals above record what each was and how it closed.
