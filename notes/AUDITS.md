...
| [2026-07-27-deck-touch-audit](2026-07-27-deck-touch-audit.md) | the **deck audit re-run crossed with touch** — the most rotted round on the menu (2,608+/1,213- across `deck.rs`/`deck.js`/`deck.css` in 49 commits, and the mode-model reshaped underneath it) | **two real defects + one consistency gap, all three SHIPPED; two rulings filed as items 70-71.** DT-1: the code-copy button is `opacity: 0` on **every** deck on **every** touch device — MOB-4's fix is `.tali-copy` at (0,1,0) and the deck declares `.tali-deck .tali-copy` at (0,2,0), and **a media query adds no specificity**, so reordering `base.css` could never have reached it. DT-2: the deck never got MOB-7's 44px tap-target floor (34×34 controls, 30.6px menu rows). **DT-5 is the one that matters most and is not a touch defect at all**: at any aspect ≠ 16:9 the adjacent slides paint into the fit slack **at rest** — 17.9% of a landscape phone's viewport, 5% on a 16:10 laptop in the stack row — found only because a landscape phone is the widest-aspect device anyone owns. Traps: a `--exact` filter with a short test name runs **zero** tests and scored 0/5 mutants killed on tests that kill 5/5; and a rect filter testing one edge rather than intersection reported DT-5 as "nothing there" |
