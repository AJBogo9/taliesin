...
files, the subdirectories, `AUDITS-archive.md`, `CUT-PROGRESS.md`, `FEATURE-IDEAS.md` and the
handover files (git keeps them), keeping `backlog.md`, `DO-NOT-REBUILD.md`, `LESSONS.md`,
`ROADMAP.md` and this index. That deletion is the author's call. Until it is made, a round adds
one dated file and one row here, and nothing else.
...
| [2026-07-27-deck-touch-audit](2026-07-27-deck-touch-audit.md) | the **deck audit re-run crossed with touch** — the most rotted round on the menu (2,608+/1,213- across `deck.rs`/`deck.js`/`deck.css` in 49 commits, and the mode-model reshaped underneath it) | **two real defects + one consistency gap, all three SHIPPED; one surviving ruling, item 71 (the second, DT-5, was filed as an item and then retracted — see below).** DT-1: the code-copy button is `opacity: 0` on **every** deck on **every** touch device — MOB-4's fix is `.tali-copy` at (0,1,0) and the deck declares `.tali-deck .tali-copy` at (0,2,0), and **a media query adds no specificity**, so reordering `base.css` could never have reached it. DT-2: the deck never got MOB-7's 44px tap-target floor (34×34 controls, 30.6px menu rows). **DT-5 was filed as the round's biggest finding and RETRACTED the same day — it was false.** "Adjacent slides paint into the letterbox" is the exact behaviour `.tali-deck`'s `overflow: hidden` + 16:9 sizing exists to prevent, and its comment says "no peek": the probe had intersected each neighbour with the **viewport** rather than with its **clipping ancestor**, and re-measurement gives **0 px** inside the clip box. Two instrument traps, both recorded in LESSONS.md: a `--exact` filter with a short test name runs **zero** tests and scored 0/5 mutants killed on tests that in fact kill 5/5; and **`getBoundingClientRect` reports geometry, not visibility** — a box can span the screen and paint nothing |
...
| [2026-07-27-premortem](2026-07-27-premortem-audit.md) | **Wave 1 / R3** — Klein pre-mortem: "it is mid-2027, Taliesin was published and it failed, why?" Prospective hindsight, the one round whose cognitive stance is opposite to every other | **77-entry raw unfiltered list kept verbatim as evidence**, three framings (nobody adopted / adopted then left / succeeded then unmaintainable), 10 ranked clusters. Top three: the evaluation funnel has one very expensive step (343-crate release build, Unix-only, no binaries, no demo, one-shot launch attention); bus factor 1 with scheduled interruptions; contribution physically impossible. **One finding died on verification** — a live jsdelivr URL in the binary is a never-reached fallback and OFF-2 already fixed it 2026-07-22; the surviving residual is coverage only (item 86) |
| [2026-07-27-untrusted-document](2026-07-27-untrusted-document-audit.md) | **Wave 1 / R5** — threat-model `.tmd` as *a format that arrives from elsewhere*. The 2026-07-17 round audited Taliesin as a **program**; publication creates the untrusted-document condition | **Three HIGH, all re-verified from source by the controller.** `--no-exec` is documented "preview untrusted docs safely" but `crates/core` has **zero** `TALIESIN_NO_EXEC` references, so `{js}`, raw `<script>` and header injection still run (item 79). `mounts:` does `root.join(&m.path)` with no containment and `Path::join` replaces the base on an absolute arg (item 80). `check` — the kernel-free pass an agent runs first — spawns the binary named by the project's own `_site.yml` `python:` field (item 81). **Position: informed consent, not safe-by-default**; `SECURITY.md:38-41` already says so and the defect is discoverability. **CSP ruling stands — no CSP, no sanitizer, no cell sandbox** |

| [2026-07-28-deck-exemption](2026-07-28-deck-exemption-audit.md) | **Wave 2 / R14** — the deck's exemption from the project's own quality machinery (not its behaviour; the 07-12 and 07-27 rounds did that) | **Items 109-113.** The scoping premise ("decks skip two diagnostic families") was too generous by an order of magnitude: an `{{< embed >}}`-referenced deck skips **all thirteen**, because `site/mod.rs:359` removes it from `site.pages` to keep it out of nav and `check.rs:295` walks that same set. `check .` and `build . --strict` both exit 0 on a deck carrying two missing assets, a broken link and unrenderable math, and the built page ships them; `check docs/guide` never mentions `demo.tmd` or `tour.tmd`. **The two documented exemptions are CORRECT** — a naive duplicate-heading rule is 100% false positives on the `auto-animate` idiom (3 hits, all deliberate magic-move pairs, measured) — what is missing is their replacement checks. Also: zero browser tests of `deck.js`, and the QR encoder is the exception with a genuine golden net. **Lesson: a dense do-not-touch cluster is not coverage, and neither is a written-down exemption list — scope from what the code *reaches*, not from what it *excludes*** |
| [2026-07-28-atam-architecture](2026-07-28-atam-architecture-audit.md) | **Wave 2 / R6** — SEI ATAM, 14 quality-attribute scenarios over the three load-bearing goals plus offline / single-surface / HTML-only | **Items 114-116, and the register is the product.** The three goals are architecturally sound: click-to-source measures **90-97%** on eight real documents with the shortfall entirely generated chrome (nine deliberate `sourcepos: String::new()` sites); a prose book consumes **no kernel at all** (the AP3-1 bypass lane plus lazy `Kernel::start`), so `corpus/tarn`'s 14 cell-free chapters never touch the pool. **Five sensitivity points** recorded, incl. that `diff.rs`'s block-id derivation is load-bearing for QA1 *and* QA2 together. **Four tradeoff points**, three correctly resolved; the unresolved one (positional cascade vs DAG) was then closed by R10 |
| [2026-07-28-fmea-detection](2026-07-28-fmea-detection-audit.md) | **Wave 2 / R7** — FMEA with the **detection** axis AP11 never scored; consumes R14's register | **Items 117-119, and a re-ranking that matters more.** **14 of 17 enumerated failure modes score D ≥ 8.** Measured zero tests for `mounts:` containment (the three "mounts" files are two false positives plus a `map` JSON assertion), zero for the `MAX_WARM_PAGES` LRU order, and one for `TALIESIN_NO_EXEC` covering `read --run` rather than the documented preview promise. **This answers the slate's founding question**: Wave 1's three HIGH findings survived ~30 correctness rounds because correctness rounds read code and this class only fails when something *runs*. Re-ranked the launch blockers by severity-with-detection: **80 and 83 now rank above 79** |
| [2026-07-28-first-contact](2026-07-28-first-contact-audit.md) | **Wave 3 / R2** — a real cold start (`env -i`, no `TALIESIN_PYTHON`, no launcher, no kernel) then Nielsen + cognitive walkthrough | **Items 120-123.** A stranger following the tool's own printed instructions **in the order it prints them** reaches a broken result in under two minutes: `init` says `preview .`, `new deck` writes into the site root and says `preview ./my-talk.tmd`, and the first instruction warns the deck will flatten. Browser-verified: the scaffolded slide reading "Each `##` heading starts a new slide" renders as one stacked article. The warning also names `revealjs`, which `is_reveal_format` rejects and which appears **zero** times in `docs/`. Plain `check` says "no problems found" on a document whose only cell cannot run. **`doctor` and the front-matter typo suggestions came back exceptional** and nothing in the round improved on them |
| [2026-07-28-conformance-acr](2026-07-28-conformance-acr-audit.md) | **Wave 3 / R9** — first conformance tool ever run against built output (Lighthouse/axe), both themes, desktop + mobile, plus a publishable ACR | **Items 124-126.** Accessibility **100** on a book chapter (both themes), a deck and the blog — and verified **non-vacuous**: `color-contrast` ran in binary mode with zero violations, discharging the deferral `a11y.rs` states in its own docstring. **One real AA failure is invisible in the score**: SC 2.5.3 Label in Name on the search button (visible "Ctrl K", named "Search"), 50 of 54 pages from `chrome.rs:50`, desktop-only because `site.css:210/239` hide the hint when narrow — and Lighthouse weights that audit **0**. Also: **12 of a deck's 14 slides are `inert`**, so a conformance tool sees ~14% of a deck. **Third mechanism to stop at the deck boundary.** Deliverables: a draft ACR and the static-vs-browser rule boundary |
| [2026-07-28-external-document](2026-07-28-external-document-audit.md) | **Wave 3 / R11** — a real external document; **L6 is no longer blocked** | **Items 127-130 plus the shape inventory.** Two real documents: `rust-lang/book` (112 files, **25,962 lines — ~2.5× the whole corpus**) and a real 8-chapter Quarto book. Both failed the same two ways and **neither is a renderer bug**: 329 of 457 problems are comma-separated fence attributes (` ```rust,ignore `, the shared convention of mdBook/rustdoc/Pandoc/Docusaurus/GitHub) reported as *spelling errors* while 11 of 18 code blocks on one real chapter ship unstyled; 118 of 123 errors are links keeping their old extension after the rename a stranger must do, whose correct target the page registry already knows. **Healthy: 112 pages built in 427 ms, exit 0**, and the `_quarto.yml` detection works on a real Quarto book, so L4's fix is verified against a real document rather than a fixture |
| [2026-07-28-author-value-stream](2026-07-28-author-value-stream-audit.md) | **Wave 3 / R8** — Lean value stream over a **real** 439-line corpus post with 3 `{python}` cells (not a fixture) | **Items 131-134, and a number that retires a question.** Browser-measured with a `MutationObserver`: a prose edit reaches the DOM in **90 ms** shipping one block op and re-running zero cells; a cell edit responds in 90 ms and settles at **152 ms**, so **the author never waits on the kernel to see their own text**. At ~200 saves an hour that is **0.8% of a writing hour** — the warm loop is not the bottleneck and cannot become one, so **further warm-loop optimisation has no value**. Closes a gap this file recorded as unmeasured: the freeze per-edit rewrite is **2.97 ms median** (~3% of the loop) on the largest real page, and the cache removes **80%** of a cold build. The author-side half is left as five questions rather than guessed |
| [2026-07-28-green-software](2026-07-28-green-software-audit.md) | **Optional tail / R13** — SCI-shaped, measured locally, no telemetry added | **Item 137.** Per-build and per-page-view are strong and claimable (**4.9 ms CPU/page** at 49 MB RSS for a 112-page book; **19 KB** HTML plus one cached CSS/JS pair; zero third-party requests). Per-deploy is not: **85% of a site build's asset bytes (4.06 of 4.75 MB) are referenced by zero pages** — `mermaid.js` and `jslibs.js` ship into a book containing neither. A deploy/storage cost, **not** a transfer cost. **Independently corroborates the concurrent branch's finding on a different project — one item, not two.** Verdict: do not publish a carbon number, and fix 137 before claiming efficiency at all |

| [2026-08-01-feature-value-audit](2026-08-01-feature-value-audit.md) | **the portfolio lens** — not *does a feature work* (60 rounds did that) and not *is the tool wanted* (PMF / demand probes / positioning did that), but **what earns its keep**: rank every user-facing feature for keep / improve / cut. Author-requested | **Items 202-209.** Adoption measured across all **185** `.tmd` documents, split author-docs (33) vs the dogfooded manual (40) vs pin fixtures (112), plus 6,265 lines of the author's shell history and a real 26-page build. Headline: **the front-matter vocabulary is the most expensive surface per unit of value in the tool** — 33 `KNOWN_KEYS`, the author's real documents set **15**, and a new key trips **six** drift gates; the fix is to promote "derive, don't declare" to a standing constraint (item 207). Second: **build effort and author demand have diverged** — ~3,300 LOC and 13.4 MB of payload landed in three weeks (research-publishing, print/PDF, pyodide, glsl, num) into surfaces with **one document each**, while the most-adopted novel capability (`{{< input >}}` 50 uses, `{js}` 40 docs, `{{< embed >}}` 33, `{{< video >}}` 27) got none. Cuts named with measurement: `columns` (**1** genuine use in 185 docs, in a doc that also uses `layout-ncol`), `datasets:` (**0**, while its shortcode has 11), `prose-lint` (**0** in five weeks). **Healthy, do not re-derive:** conditional asset bundling works (mermaid 3.5 MB loads on **2 of 26** pages, jslibs 1, katex 4, deck 2) — the real payload cost is the **101 MB binary** carrying pyodide's 12.9 MB via `include_bytes!`, which is the evaluation funnel the premortem ranked top-three. Highest-value gap (F5/item 202): **the tool cannot answer "what does this document use"**, though the pipeline already knows — which would make corpus-plus-roadmap self-checking and every future portfolio audit free. **Two instrument traps hit and corrected mid-round**, both recurrences: an all-zero adoption table (zsh does not word-split unquoted parameters — caught by this file's own broken-probe rule) and "mermaid loads on 24/26 pages", which was the **inlined-asset needle trap** (the word in bundled-JS comments); needling the real `<script src>` tag gives 2/26. **Not measured:** reader-side value (no telemetry, by stance) and the ~11,300-LOC `lsp*.rs` surface, which history structurally cannot see and which deserves its own round |
| [2026-08-02-final-scope-audit](2026-08-02-final-scope-audit.md) | **the final-scope lens** — the FV-2 ablation successor crossed with cost-to-carry, run across the WHOLE surface rather than the three features already filed. Author-requested, to define a finished product before publishing | **Spec: [2026-08-02-final-scope-design.md](../docs/superpowers/specs/2026-08-02-final-scope-design.md).** 501 features catalogued across nine disjoint surfaces, then four cross-cutting lenses (moat, inherited-vocabulary, maximal-cut skeptic, keep-defender); 13 agents, 2.96M tokens, 0 errors. Owner rulings: audience = **a real product for strangers**, aggressiveness = **scope B of three**, decks **frozen not cut**, headless-Chrome family **only the math hover cut**. Plan: **~6,900 LOC (5.3%) and 16 MB** over six waves; 21 subcommands to 13, 73 config keys to ~58, two dev servers to one, all four formats retained. **Headline, and the biggest launch risk: the published moat number is wrong by 9.3x** — `RESULTS.md` says "83x smaller payload", `RESULTS.json` on the same machine gives 32,303/287,195 = **8.89x** with `update_count 1` where the committed file says 0; it is gitignored so nothing regenerates it and the test asserts only a 5x floor, and the 83x figure is cited as fact in ROADMAP.md, backlog.md, AUDITS.md and the launch critique. Second: **the moat is 13% of the tree** (~17,000 of ~131,000 LOC) and **maintenance is inverted against the thesis** — the deck engine has **178 commits** against **28** for `diff.rs`+`freeze.rs` combined. Third: **the inherited-vocabulary hypothesis was refuted by measurement** — inherited and chosen features are cut at an identical rate (17.4% vs 17.3%) because the shed-Quarto ruling was genuinely applied; the 71% that was never inherited has never had an equivalent ruling and carries **3x the freeze rate**, so the bloat is in what was invented, not what was copied. **Also found: `taliesin features` panics on 3 of 25 guide pages** (`render/extension/mod.rs:135` advances a byte cursor by 1, `:124` then slices mid-codepoint — any line with `{{<` plus a non-ASCII char). **Seven cuts REJECTED on evidence** and recorded so they are not re-derived: social cards (a named regression test forbids the proposed replacement), `publish` (the only proven real-use trace in the audit), `tali.tex`/`tali.table` (byte claim wrong by 4x), companion terminal links, `projectRefs`, pyodide-as-deletion, decks. **Three prior claims corrected:** the "2 real theorem docs" was false (one doc, one of eight kinds), the **101 MB binary** is 72.1 MiB on disk and **32 MiB downloaded**, and `logo:`/`footer:` are site-level not dead |
| 2026-07-28-public-flip-audit | **Phase 1 of the public flip** — the read-only pre-publication audit across ten dimensions (purge set / leaked content / commit messages / secrets / personal data / licensing / corpus provenance / tone / readiness / history hygiene), run 2026-08-03 against `62a3908e`. Spec: [2026-07-28-public-flip-audit-design.md](../docs/superpowers/specs/2026-07-28-public-flip-audit-design.md) | **61 findings; Phase 2 NOT started.** Method: `--batch-all-objects` over **20,450 objects**, 7,290 text blobs line-scanned (2 named skips), so D1/D2/D4 cover the full object graph rather than `HEAD`. **The load-bearing output — D2's verdict — is `--replace-text` across all history, not a link-repair commit**, and both prior claims were partly wrong: the majority of `notes/backlog.md`'s money-word hits really are estimation vocabulary ("price a change by making it"), but **seven distinct restatements of the purged docs' commercial conclusions exist only in history** (quoted in the findings file, redacted here), where a repair commit cannot reach them; two sit at paths already absent from `HEAD` and are cheaper to add to `--invert-paths`. **Two dimensions came back genuinely empty and say what they searched** — D4 (zero credentials: 16 provider token shapes, assignment shapes, and 13 secret-shaped filenames, all clean across history; the only hits are `hunter2` fixtures and env-var *names*) and D8 (zero swipes: every named individual in the tree is a copyright attribution, and the one "lazy" hit is a *defense* of Quarto). **The most consequential finding is outside the money set: D7:** the original `corpus/bayesian-website/` was joint work with a named co-author, credited by name in its own text, and it bundled a university's own assignment brief verbatim: ~60 lines of course staff's copyrighted text, with no licence and no rendering role. Neither was the author's to publish under AGPL-3.0 alone. It was replaced with `corpus/single-page-report/` on `HEAD` and its history is being removed from both repository copies entirely (ruling D-8, 2026-08-20). **Counts re-measured against the 2026-07-28 spec:** 1,851 commits not 1,608; `.git` 56 MB not 68; 14 by-name references not 10 (but **5 are themselves purge-set members**, so only 9 need action); 19 `/home/bogo` files not 14; 9 commit *subjects* plus a class the spec missed — **3 commit bodies**. D9 confirmed: all 8 CI jobs inert since restore (last real run had **`kernel-r` failing**), `release.yml` also guarded so **tagging while private produces nothing** (flip must precede tag), and the README promises binaries from three "built and released" platforms against **zero releases**. **One open question surfaced, not answered:** product-for-strangers vs portfolio-for-employers re-ranks D9 from launch-blocking to cosmetic; D1-D8 and D10 are invariant to it. **Traps:** a `cat-file --batch` scanner deadlocks writing SHAs to stdin before reading stdout, and a Bash cwd reset made one scan return nothing — which reads as "clean" and was the exact opposite |

| [2026-08-04-fv5-instrument-and-portfolio-recheck](2026-08-04-fv5-instrument-and-portfolio-recheck.md) | **FV-5 built, and the portfolio re-measured at HEAD** (`253fa173`, i.e. after Waves 0-6 and the minimalism pass), plus the author's question of whether the books and marketing site are in good condition. Author-requested | **Cut nothing; its one proposed cut was refuted by its own evidence.** Ships `crates/server/src/lsp_trace.rs`: `TALIESIN_LSP_TRACE=<path>` tallies dispatched LSP method names, unset is off, nothing leaves the machine, and it **counts rather than logs** because the window is a week of writing across many editor restarts — the tally is seeded from the file at startup so restarts accumulate, and `sessions` distinguishes *never invoked* from *never armed*. Verified by driving the real `taliesin lsp` over stdio, not only by unit test. **The finding neither prior round measured: test code is 48% of the Rust** (58,868 of 122,861), so every feature carries ~1 test line per implementation line on top of its drift gates, and the executed waves bought roughly double the LOC they booked. LSP re-priced at **15,592 LOC including tests (impl 6,441), largest feature by 2.8x**, still the only unmeasured one. `preview` 58 invocations vs 0 for `pdf`/`run`/`new`/`doctor`. **The docs are NOT stale**: 16 deleted-feature greps hit 11 files and **every hit is a false positive** ("taliesin **renders**", "the **dev server**", "**skim**ming", the lightbox *removal register*, and `from-quarto.tmd` where a retired key must tell a migrating reader what to do); `stale_docs.rs` green on all 7 gates. **The withdrawn cut, recorded so it is not re-derived:** folding `site/formats.tmd` into `index.tmd` was proposed from **headings alone** and reversed by the inbound-link graph — `index.tmd`'s section is a four-card grid linking *into* `formats.tmd`'s anchors, and the page carries 10 inbound links, a nav slot, the live deck embed and a `stale_docs.rs` entry. Each of features/formats/showcase/gallery answers a *different* question. **Traps:** `~/.zsh_history` is non-ISO extended-ASCII so grep treats it as binary and the first CLI table came back all zeros (third recurrence — carry the control row); and a page's role is in its inbound links, not its headings |
| [2026-08-07-devtooling-frontier-audit](2026-08-07-devtooling-frontier-audit.md) | **the editor-and-machine lens** — `taliesin lsp`, the companion, `taliesin mcp`, the CLI diagnostic surface and the contributor loop, benchmarked against the LSP 3.17 metaModel, the MCP 2025-11-25 server spec and the Zed/Helix highlighting reality. **This is the round FV-2 named as its biggest blind spot and FV-5 re-priced at 15,592 LOC, largest feature by 2.8x, still unmeasured.** Author-requested | **10 findings; 214, 215, 218 and 219 built 2026-08-07 (Tier 0 plus the two companion items), and finding 5's owed doc-narrowing landed the same day (backlog 221: `cli.tmd` and `validation.tmd` now say diagnostics-and-intelligence, not highlighting, and ship the Helix wiring; building the highlighting stays ruled out). The rest open.** Method: the **release binary driven over real stdio** by two committed probes ([harness](2026-08-07-devtooling-harness/)), each carrying a positive control. **Headline, and it is an inversion of the project's own invariant: the editor gets a strictly weaker diagnostic than the read-only preview.** Measured on a 2-page site, `taliesin lsp` publishes 5 of the 6 codes `check` finds — it misses `TAL-LINK-ANCHOR` entirely and downgrades `TAL-LINK` to the standalone phrasing — because `check.rs:312` lints every buffer with `Scope::Standalone` and never calls `cross_page_diagnostics`, **which `serve_site/mod.rs:1852` already calls and which `preview_diag.rs:37` already made incremental** ("work that does not grow with the size of the book"). So the 2026-07-18 DX audit's #1 finding, the validation cliff, was closed on the read-only view and left standing on the single editing surface — and `cli.tmd:625`'s "the same validators as `check`" is measurably false for any page in a site. Second: **`client.ts:65` watches `**/{*.tmd,_site.yml,*.bib}` and the server drops `workspace/didChangeWatchedFiles` on the floor** (zero occurrences; the `workspace` capability block serializes to `null`), so a `git pull` or a `.bib` edit leaves every open buffer stale. Third: **the five LSP methods that best fit a cross-referenced format are the missing ones** — `references` (already computed, published only on the proprietary `taliesin/projectRefs`), `codeLens` (the highest-leverage single gap: *Run cell · Run above · ⚡ cached* in every editor, from Rust, zero TS), both halves of the 3.17 pull model, `selectionRange`. Fourth, and **the obvious answer is wrong**: `.tmd` has **zero tree-sitter grammar and zero semantic tokens**, so Helix/Zed get diagnostics over unhighlighted grey text — and semantic tokens (the one-source-of-truth route) **cannot** fix Helix, which has no support, or default-Zed, where the setting is `off`; three options are laid out, the doc-narrowing one is owed regardless. **Measured healthy:** 144 ms `didChange`→`publishDiagnostics` (120 of it the deliberate debounce), sub-ms symbols/folding/formatting, <10 ms startup, 0.36 s `check` on 22 pages. **The outlier is `workspace/symbol` at 167 ms** with no `$/cancelRequest`, so every Ctrl-T keystroke queues another full project walk that cannot be abandoned. **Trivial + highest evidence-per-minute: `TALIESIN_LSP_TRACE` has never been armed** — no trace file exists on the machine, so FV-5's instrument has produced nothing and the largest feature is *still* at zero adoption evidence. Research-grounded, not persona-grounded: CHI 2020's four high-impact notebook activities put **long-running tasks** in the set (76% important / 55% difficult, "no feedback on progress"), which Taliesin answers **in the browser only** (`exec.rs:113` `ProgressSink`), and **Reproduce-and-Reuse**, which `freeze.rs:19-29` already documents honestly as unclosed (an in-place `pip install --upgrade` moves no key). **Not measured, and named as the natural successor:** completion *quality* (the most-touched capability), and the `.tmd` dialect under Cognitive Dimensions — the correct academic lens for a notation, never applied |
| [2026-08-13-mvp-audit-backlog](2026-08-13-mvp-audit-backlog.md) | **the post-cut lens** — is the surface the 13 cut waves left actually a sound MVP, and what is broken in what survived. Two tracks in one run: 5 scope lenses (author journey, residual bloat, moat integrity, code/surface proportionality, first-contact readiness) and 8 subsystem bug finders, with one adversarial refuter per finding. Author-requested | **The findings file IS the work queue; items are deleted from it as they land.** 48 agents at `aceb566b`, 4.5M tokens, 0 errors. 34 candidate defects, **2 refuted**, **29 distinct confirmed**; 36 scope findings, 16 surviving synthesis. **Verdict: the scope is right and the shipping surface has not caught up.** All five lenses independently returned sound-with-caveats; the seven-verb journey has no hole, all three load-bearing goals were exercised **live** (click-to-source through an include/div/executed-image; `update 1 block` with `{js}` runtime state preserved; warm edit costs nothing), and an independent scan found the document vocabulary **fully witnessed** — the recorded "10 unused offered names" tail is **stale by eight**, the real tail is `head:` and `python:`. **So: do not cut another feature.** **Headline defect, and it is data loss: `build <project> --out <dir>` deletes files in the output directory and exits 0** — the in-place guard tests only `root == out` and `sweep_stale` has no notion of whose directory it is, so `build blog --out public` ate `CNAME`, `thesis.txt` and `photos/cat.jpg`, and `build myblog --out .` ate `README.md`, `src/main.rs` and both `.tmd` sources, reporting `swept 4 stale files`. **Second: the pre-publish gate is not a gate** — `--check-only --strict` prints "no problems found" on a link to a `draft:` page (ships a 404), on a broken `_site.yml` nav href (ships on every page), and on a directory that plain `build` refuses; and `docs/internals` is built but never linted in any of the three gate files. **Third: the freeze cache can publish a wrong number** — deleting a `#| cache: false` line permanently freezes a self-contradictory page (`upstream x = 451379` / `downstream sees x = 837111`, stable across rebuilds) because `strip_cell_options` drops `#|` before hashing, and `stamp_packages` runs before `warn_if_packages_moved` so the package-change warning can never fire. **The sentence that carries the scope verdict: `README.md:157-165` advertises four removed features and `retired_names.rs:294` is named `the_lightbox_is_gone_from_the_client_bundle`** — the test suite asserts the absence of what the README sells, shipped in every release tarball, for nine days with every gate green. **Every defect exists in a tree where `cargo test --workspace` is 81 suites / 1,352 passed / 0 failed**, which is the finding behind the findings: the cut removed features and their tests together, correctly, but added no coverage where the remainder became load-bearing. **The refuter pass earned its cost:** it killed 2 findings and corrected the severity, trigger, root cause or anchor of **at least 14** more (one obvious fix would have reintroduced a wedge; one severity went *up*, a `?query` link leaking `draft:` source into the deploy). **Measured healthy, do not re-derive:** production Rust is 35,386 lines against 42,707 test lines with **zero traits and two one-variant enums**; the alarming file sizes are mostly test modules (`lsp.rs` is 1,742 production of 4,138); `corpus/tarn` is **not** cuttable (composed into the deploy, and the only golden for cross-page `@sec-` numbering). **The real outlier is `notes/`: 36,315 lines across 104 files, larger than `crates/core/src`, exempt from every drift gate, containing a 1,222-line unmarked copy of the pre-cut diagnostics vocabulary** |
| [2026-07-27-lsp-l3](2026-07-27-lsp-l3-audit.md) | lens L3, editor-facing half: `lsp.rs`, `lsp_complete.rs`, `complete.rs` | dated record; superseded by the 2026-08-08 scope ruling and the cut (row added 2026-09-24) |
| [2026-07-28-vscode-companion](2026-07-28-vscode-companion-audit.md) | the VS Code companion and its completion surface, measured by driving `detectContext` and unpacking the `.vsix` | dated record; superseded by the 2026-08-08 scope ruling and the cut (row added 2026-09-24) |
| [2026-08-17-fable-audit-backlog](2026-08-17-fable-audit-backlog.md) | 16-lens fresh-model audit at `5b9684ae`; the refuter fleet was stopped by a spend limit (26 findings completed the cycle: 20 confirmed, 6 refuted) | its DECIDE calls were ruled 2026-08-17 (CLAUDE.md trim, registers cut); row added 2026-09-24 |
