...
| [2026-07-28-launch-critique](2026-07-28-launch-critique.md) | **pre-launch adversarial round: 6 critics on disjoint surfaces, a defender per critic that had to refute; ~100 findings adjudicated** | **14 commits MERGED 2026-07-28** (deck source leak, LSP `languageId`, the stale-prose gate twice, the AGPL/MIT contradiction, false CI claims, the landing page's imaginary pen tool, ~10 further false doc claims). The rest became backlog items **138-149** — issued as 79-90, **renumbered in the merge** because Wave 1 already held 79-108. Two were duplicates found independently by the same-day audit waves and are retired into the earlier number: its 88 → **101** (output licence) and its 86 → **137** (unreferenced `_assets/`), both cases where the critique round had the sharper mechanism and its text was folded in. Its own findings doc uses internal IDs (`BL-1`, `CJ-1`, …), so nothing in it needed rewriting |
...
| [2026-07-27-adoption-friction](2026-07-27-adoption-friction-audit.md) | **Wave 1 / R1** — JTBD four forces: audit for *anxiety and habit*, not appeal. The four demand probes all measured *pull* (can a persona finish a task); the switching equation is `Push + Pull > Anxiety + Habit`, so one term of four had ever been measured | **items 79-108 batch.** Headline: **zero** statements anywhere in source about what a user's *output* is licensed as, while every built page inlines bundled CSS/JS carrying no licence header (item 101, an owner ruling). **The "no exit path" anxiety is REFUTED by measurement** and that is worth more than fixing it: across all 115 corpus docs (10,118 lines) at most **8.59%** of lines carry any non-CommonMark construct, all six families Pandoc/Quarto vocabulary, and three exits already exist unnamed (item 94). `taliesin check` already *is* a Quarto migration assistant and nothing says so (item 93). Corrected the dispatching brief: `corpus/tech-blog/` is the personal blog, `site/` is the marketing site |
...
| [2026-07-27-due-diligence](2026-07-27-due-diligence-audit.md) | **Wave 1 / R4** — technical due diligence + contributor readiness: what an acquirer, grant committee or serious contributor finds, measured on a **fresh clone** | **The gate-rot hypothesis is REFUTED — all four hand-run gates PASS** (live Python 457 tests exit 0 and non-vacuous; live R 3/3 in 20.4 s on a real IRkernel boot; both `tsc` exit 0, `--listFiles` confirming 5 and 25 files; `node --test` 6/6; plus `cargo audit` and `cargo deny` exit 0). What is missing is anything making an *outsider's* run non-vacuous — the gates skip silently (item 84). Red flags: three tracked files claim MIT and **tag `v0.2.0` genuinely ships an MIT `LICENSE`** while HEAD is AGPL (items 82, 83); the two self-flagged private strategy notes are git-tracked against their own instructions (item 100); `core.hooksPath` is **unset in a fresh clone** so a PR runs no gate. **Trap:** `cargo test --lib` on `taliesin-server` errors (it is a **bin** crate) and `cmd > log; echo $?` reported exit 0 while the gate had not run |
...
| [2026-07-28-demand-positioning](2026-07-28-demand-positioning-audit.md) | **Wave 3 tail / R10** — real user language from Quarto + marimo trackers, every candidate verified against Taliesin's own engine first | **Items 135-136.** The shipped-but-unmarketed list is the longer one, as the slate predicted, and now every entry is a real user asking for what Taliesin already does (marimo #3114/#1379/#2675, Quarto #4201/#3674/#10429). **The strongest is unclaimed by anyone**: Quarto's tracker carries *stale-output* complaints against `freeze`, and Taliesin's cumulative key makes a stale hit **structurally impossible**. **Resolves the slate's sharpest thread — tell the cascade story, do not build the Python DAG** (closing item 116). Only two ecosystems surveyed; Slidev/Observable/VitePress/mdBook were not |
