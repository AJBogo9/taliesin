...
All four of these are green today. A2, A3 and A4 print `no problems found` and exit 0 under
`--strict`.

## A2 [V] HIGH: `--check-only` passes on a directory that `build` refuses

`crates/server/src/build.rs:289` dispatches `--check-only` before the is-a-project check at
`:298`, and `crates/server/src/lint.rs:419` independently filters the missing-config warning
out. Either fix alone closes the reported case; fix both.

```sh
mkdir -p /tmp/noyml && printf -- '---\ntitle: X\n---\n\nHi.\n' > /tmp/noyml/index.tmd
./target/release/taliesin build /tmp/noyml --check-only   # "no problems found", exit 0
./target/release/taliesin build /tmp/noyml                # refuses, exit 1
```

CI runs the documented gate, gets green, and the publish step then fails. This is the exact
failure mode wave P3 was written against.

**Second consequence the finder omitted:** because the guard is skipped,
`collect_site_diagnostics` treats *any* directory as its own project.

## A3 [V] HIGH: a link to a `draft: true` page passes the gate and 404s in the deploy

`crates/core/src/site/mod.rs:803-805` (the **second** disjunct; line 802 is the legitimate
raw-asset case and is pinned by `crates/core/tests/migrated_link_extensions.rs:115`, so a
fix must not touch it).

`resolve_link_warnings` excuses any target whose `.tmd` exists on disk, but drafts are
stripped from `self.pages` at discovery, so the link resolves to an `.html` the build never
writes.

```sh
# index.tmd contains [the post](posts/a.tmd); posts/a.tmd has draft: true
$ taliesin build . --check-only --strict
  info    1 draft not published: posts/a.tmd
no problems found
EXIT=0
$ grep -o 'href="posts/a.html"' out/index.html   # present
$ ls out/posts                                    # no such directory
```

**Wider than drafts:** the same hatch excuses a link to any `.tmd` on disk that discovery did
not publish. A book whose `_site.yml` lists `chapters: [index.tmd]` with an unlisted
`stray.tmd` beside it produces no "N drafts not published" line at all, so that variant is
strictly worse.

## A4 [V] HIGH: a broken `nav:`/`footer:` href is never link-checked and ships on every page

`crates/core/src/site/mod.rs:630`. Cross-page link validation harvests links only from
rendered page bodies (`page_link_facts_from_src`), so hrefs configured in `_site.yml` chrome
are emitted by `chrome.rs` without ever being resolved against the page registry.
...
no problems found
EXIT=0
$ grep -o 'href="missing.html"' out/index.html   # present, on every page
```

**Two collection sites, not one:** `nav_link` and the `footer_html` group closure at
`crates/core/src/site/chrome.rs:243-259`. `nav_link` drops an href-less item and
`footer_html` renders it as a `<span>`, so only href-bearing items are affected. The same
hole covers a nav href pointing at a draft.

## A5 [A] MEDIUM: `docs/internals` is built but never linted

**Latent, not live:** `build docs/internals --check-only --no-exec` on the current tree
prints "no static problems found" and exits 0. The book is clean today. What is missing is
the gate that would catch the next broken cross-reference.

The only automatic thing touching that book is `tools/build-site.sh --check`, which runs a
plain `build <src> --out <dir> --no-exec`; that exits 0 on any number of errors.

**Do not blame `tools/build-site.sh:70`.** The finder mis-anchored this. That line is a
compose-and-build step doing its stated job. The defect is an **absence** in three files:
no `build docs/internals --check-only --no-exec` step at `.githooks/pre-push:90`,
`tools/gates.sh:337`, or `.github/workflows/ci.yml:73`. Wave 9 wired the gate for
`docs/guide` and covered one of two already-separate books.
`crates/core/tests/gate_script.rs:175`'s substring check for `"--check-only"` is
structurally incapable of noticing which book is named.

**Fix:** one line in each of the three gate files. Consider whether `gate_script.rs` should
compare the *set* of linted book roots against the set of `_site.yml` projects under
`docs/`.

**Done when:** appending `See @fig-does-not-exist.` to `docs/internals/index.tmd` turns
`./tools/gates.sh` red.
