...
**Pick up here (2026-07-19, polish audit — ALL PL1–PL20 LANDED + pushed to `origin/main`).** A
feature-*polish* audit ran (4 read-only auditors: CLI/DX · authoring · live view · theming, + online research +
per-finding source verification; full findings + credit in
[2026-07-19-polish-audit.md](2026-07-19-polish-audit.md)). Distinct lens from every prior audit
(DX/PMF/machine-facing/vacuous-test/website/deck): "can an existing feature be *simplified with the same power*,
or is it *implemented unintuitively*?" **All 20 items shipped in two batches, one commit each on `origin/main`:**
Batch 1 (PL1/2/3/7/8/9/13/14/15/16 + the empty-div grouping fix, `86d404e`…`06fa302`) and Batch 2
(PL4/5/6/10/11/12/17/18/19/20, `8eeb91a`…`76185be`). Verify with `git log --oneline origin/main | grep PL` (the
author may push again mid-session, so re-check, never trust a recorded SHA); batch closure record in
[AUDITS.md](AUDITS.md). **What remains in §7:** only the four owner-ruling **Design questions** (deck serif/sans,
focus-mode↔fullscreen coupling, a `//| uses:` alias, theorem-kind namespacing) — none build-ready without a ruling.
...
### 7. Polish audit batch (2026-07-19) — PL items all LANDED; only design questions remain

**All PL1–PL20 shipped and are on `origin/main`**, one commit each, in two batches: Batch 1
(PL1/2/3/7/8/9/13/14/15/16 + the empty-div grouping/`group_divs` fix, `86d404e`…`06fa302`) then Batch 2
(PL4/5/6/10/11/12/17/18/19/20, `8eeb91a`…`76185be`) — enumerate with `git log --oneline origin/main | grep PL`.
Full findings + per-item evidence + credit: [2026-07-19-polish-audit.md](2026-07-19-polish-audit.md); batch
closure record in [AUDITS.md](AUDITS.md). **The named finding held:** the tool validated + located almost
everything, yet a handful of features offered co-equal spellings with no canonical, or silently dropped input its
own vocabulary invited — those silent holes are now closed, the palette + geometry/motion are single-sourced
`--tali-*` tokens, exec/error boxes derive from the callout tokens, and human output no longer under-sells shipped
machinery. **Do not re-open the PL items.**

**Still open — Design questions (owner ruling first, NOT build-ready):** deck inverts the page serif/sans logic
