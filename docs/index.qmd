...
`inline code`, [links](https://example.com), lists, blockquotes, and tables with
column alignment. Code blocks are syntax-highlighted and get a hover **Copy**
button.
...
the kernel never cold-starts. Execution needs a Python with `ipykernel`
available; point the server at it with the `QMD_FAST_PYTHON` environment
variable. Without one, a cell renders as highlighted source with no output rather
than failing.

## Diagrams

A `{mermaid}` code block renders client-side with mermaid.js, loaded lazily the
first time a diagram appears on the page. The flowcharts in
[Architecture](#architecture) above are rendered exactly this way.
...
numbered figures, and clicking any figure opens it full-screen in a lightbox.
...
(citation groups and locators like `[@a; @b, p. 5]` work too). Cross-references
resolve to labelled links across the kinds `@fig-`, `@sec-`, `@tbl-`, `@eq-`,
`@lst-`, `@thm-`, and `@def-`, carrying the resolved number when it is known.

## Click-to-source and editor sync

In the VS Code extension, double-clicking a block in the preview reveals the
exact source line in the editor (including across `{{< include >}}`d files), and
moving the editor cursor highlights the matching block in the preview. The
standalone browser preview highlights a clicked block and reports it to the
server, but does not open an editor.
