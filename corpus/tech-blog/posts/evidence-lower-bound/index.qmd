...
const history = qmd.defines.history; if (!history) return;
const mu_post = qmd.defines.mu_post;
const sig_post = qmd.defines.sig_post;
const iter = qmd.value("iter");

const snap = history[iter - 1];

const z_vals = Array.from({length: 500}, (_, i) => -3 + i * 7 / 499);
...
const true_curve = z_vals.map((zi, i) => ({
  z: zi,
  y: normalPDF(z_vals, mu_post, sig_post)[i],
}));

const q_curve = z_vals.map((zi, i) => ({
  z: zi,
...
const ll_data = history.slice(0, 40).map(d => ({ iter: d.iter, elbo: d.elbo }));
