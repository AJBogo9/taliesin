...
| `deck.tmd` | Slide deck (`format: deck`) | the deck-engine regression net: headings→slides, `. . .` pause fragments + `.incremental`, code line-stepping (`code-line-numbers`), `.magic-move`, an `auto-animate` pair, a per-slide `background-color`, `.tali-stretch`, speaker notes, a vertical stack (h1 lead + h2 children), an explicit `{#id}` slide anchor, **server-rendered math (display + inline)** and a **`{python}` kernel cell** — one kept rich feature per slide, pinned by `corpus_deck_pins_every_kept_rich_feature`. It is also the fixture the deck engine's only **browser** test walks (`crates/server/tests/deck_browser.rs`: real `ArrowRight` events, hash round-trip, and the a11y scan of every slide) | (purpose-built) |
...
| `reactive/pyodide.tmd` | `{pyodide}` client-side Python | the cell-language registry's third language: a `{pyodide}` cell publishing a NumPy sample array with `#| name:`, a `{js}` cell consuming it across the language boundary through the same reactive graph, and a second `{pyodide}` cell showing `print` output above its returned value. The server-side contract (wrapper emission, asset gating, single-file degradation) is pinned by `crates/core/tests/pyodide.rs`, which uses its own fixtures rather than this file; **this** document is the one `crates/server/tests/pyodide_browser.rs` serves over HTTP and drives in headless Chrome, so it is what proves the runtime boots, the value publishes and the cross-language edge fires | (purpose-built) |
