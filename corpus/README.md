...
| `cite-this/` | The research-publishing surface | the paper page (`paper.tmd`) that pins items 184-187 end to end: structured `author:` entries (affiliation, ORCID, url, `equal`, `contribution`) with the superscript numbers **derived** from first appearance; the `venue`/`award` badges and the `links:` resource row, whose labels and icons are inferred from each URL; the generated appendix (Author Contributions / Acknowledgments / DOI); and the full Google Scholar `citation_*` block, whose ordering is load-bearing (an institution binds to the author above it). `note.tmd` is the site-author fallback and `index.tmd` the dateless no-box case. Pinned by `cite_this.rs` + `scholar_meta.rs` | (purpose-built) |
...
| `media/screencast.tmd` | `{{< video >}}` playback ladder | the three playback steps on one page: the default silent hover-preview screencast, `controls` (the browser's own player — scrubber/keyboard/fullscreen/PiP), and `audio` + `captions=` (a narrated explainer: unmuted, non-looping, caption track), plus a flag written *before* the source path. Pinned by `video_playback.rs`; `tour.mp4` is a generated 3 s fixture carrying a real audio track so `audio` is exercised rather than asserted | (purpose-built) |
...
| `reactive/animate.tmd` | `type="animate"` + `tali.state` | a play/pause/step/reset tick driving a downstream cell one frame at a time, and a cell accumulating across scheduled re-runs | (purpose-built) |
| `reactive/point.tmd` | `type="point"` | a draggable 2-D coordinate: the one structured input value (`{x, y}`, y up), refitting a cloud as the reader moves it. Exercised by both keyboard and a real pointer press in `reactive_browser.rs` | (purpose-built) |
...
| `reader/` | Reader experience | read-only reader enhancers: display prefs (light/dark theme, applied before paint), reading position + resume, hover cross-ref cards, anchor copy-links, and a read-state right-rail TOC | (purpose-built) |
