...
| `deck.tmd` | Slide deck (`format: deck`) | the deck-engine regression net: headings→slides, `. . .` pause fragments + `.incremental`, code line-stepping (`code-line-numbers`), `.magic-move`, an `auto-animate` pair, a per-slide `background-color`, `.tali-stretch`, speaker notes, a vertical stack (h1 lead + h2 children), an explicit `{#id}` slide anchor, **server-rendered math (display + inline)** and a **`{python}` kernel cell**, plus the persistent deck chrome (`footer:` and the bottom-left `logo:` corner mark, `corpus/logo.svg`) — one kept rich feature per slide, pinned by `corpus_deck_pins_every_kept_rich_feature`. It is also the fixture the deck engine's only **browser** test walks (`crates/server/tests/deck_browser.rs`: real `ArrowRight` events, hash round-trip, and the a11y scan of every slide) | (purpose-built) |
| `embed/` | `{{< embed >}}` shortcode | a page embeds a deck (`{{< embed talk.tmd title=… >}}`, whose `title=` is the iframe's only accessible name); a **site** build must build the deck as a standalone `.html` beside the page and keep it out of the nav, so the iframe resolves. Pinned by `embed_site_build.rs` (site half) + render-level unit tests (iframe markup, `.tmd`→`.html` href, `embed_targets` extraction); the single-doc *warning* half is `embed_warning.rs` | (purpose-built) |
...
| `theorem-book/` | Per-chapter `theorems:` in a book | `alpha.tmd` declares `shared: [theorem, lemma]` and draws one chapter-scoped sequence (1.1, 1.2); `beta.tmd` declares nothing and must NOT inherit it, so each kind counts from 2.1. Pins the no-inheritance property left by retiring the book-wide `_site.yml theorems:` policy. Pinned by `book_theorems.rs` | (purpose-built) |
| `structured-authors/` | Structured `author:` front matter | the paper page (`paper.tmd`) that pins items 184 and 187 end to end: structured `author:` entries (affiliation, ORCID, url, `equal`, `contribution`) with the superscript numbers **derived** from first appearance, and the generated appendix (Author Contributions). `note.tmd` pins the JSON-LD site-author fallback and `index.tmd` the dateless case. Repurposed on 2026-08-03 from `cite-this/`, whose cite-this box, `citation_*` block and `doi`/`venue`/`award`/`links`/`acknowledgments` keys were retired; the structured-author half it also pinned is what survives | (purpose-built) |
...
| `media/screencast.tmd` | `{{< video >}}` playback ladder | the playback steps on one page: the default (native `controls` — the browser's own player: scrubber/keyboard/fullscreen/PiP), `controls=false` (no control bar and no play affordance at all), and `audio` + `captions=` (a narrated explainer: unmuted, non-looping, caption track), plus a flag written *before* the source path, and a **theme-adaptive pair** (`poster=` still + `dark=` alternate source, `tour-light.mp4`/`tour.mp4` being the same 3 s inverted so the theme toggle visibly swaps them). Pinned by `video_playback.rs`; `tour.mp4` is a generated 3 s fixture carrying a real audio track so `audio` is exercised rather than asserted | (purpose-built) |
| `media/optimized-images.tmd` | Image optimization | intrinsic `width`/`height` on every local raster image, the LCP exception (first image eager + `fetchpriority`, the rest `loading="lazy"`), and a source **below the smallest width rung** — `fit-small.png` is 320 px, and a rung wider than its source measured *larger than the original*. Pinned by `image_optimization.rs`; the AVIF half is build-only and pinned in `image_opt.rs` | (purpose-built) |
...
| `reactive/inputs.tmd` | `{{< input >}}` controls | declarative reactive controls (slider/number/checkbox/text/select, plus `range` as the accepted alias of `slider`) feeding `{js}` cells through the graph (incl. a transitive chain) | (purpose-built) |
...
| `reactive/glsl.tmd` | `{glsl}` shader cells | the cell-language registry's second language: a fragment shader to a live `<canvas>`, driven by a `{{< input >}}` through the same reactive graph; one animated (`u_time`) and one static + numbered as a `@fig-`. Pinned by `crates/core/tests/client_lang.rs` (emission) and `crates/server/tests/reactive_browser.rs` (it actually paints) | (purpose-built) |
| `reactive/numerics.tmd` | the `num` global + rich output | distributions/stats/seeded PRNG/linear algebra beside `Plot`/`d3`, plus `tali.tex` (a typeset matrix) and `tali.table` (inferred columns). Numbers checked against closed forms in `reactive_browser.rs` | (purpose-built) |
| `refs/theorems.tmd` | Theorem environments | all 5 kinds across the 2 amsthm styles, `title=`, a proof with auto-QED, per-kind continuous numbering, and `@thm-`/`@def-`/`@lem-` cross-refs resolving | (purpose-built) |
| `refs/theorems-shared.tmd` | Shared theorem counters | `theorems: shared: [...]` makes theorem/lemma/corollary draw one sequence (Theorem 1, Lemma 2, Corollary 3) while `definition` counts separately; cross-refs resolve to the shared numbers. The list also pins that its VALUES are validated (`TAL-THM-KIND`): it carried a fourth kind, `proposition`, for four days after that kind was retired on 2026-08-03, and `check` stayed clean because only the `shared:` key was checked, never its entries | (purpose-built) |
| `refs/theorems-interactive.tmd` | Web-native theorem affordances | a collapsible `::: {.proof collapse="true"}` (native `<details>`) | (purpose-built) |
...
| `diagnostics/` | Validator coverage | docs that deliberately trip Taliesin's schema validators (`typos.tmd`: unknown top-level, `execute:`, `listing:` and `theorems: shared:` entries, which cover the three mistakes that need three different messages — a **typo** gets a did-you-mean, a **retired** name gets a removal note, and a **recognized but inert** key (`csl:`) gets neither) + the `check`-superset static lints (`check-superset.tmd`: duplicate `{#id}`, broken in-page anchor, missing image, citation with no `bibliography:`) + widget-shape mistakes (`widgets.tmd`: an empty tabset/walkthrough/scrolly, a nameless or unknown-typed `{{< input >}}`, and `::: {.debug}`'s own four authoring slips: no traced cell, more than one, `#\| trace: true` sitting outside any `.debug` div, and a non-boolean `trace:` value) | (purpose-built) |
...
| `reactive/state.tmd` | `tali.state` | a cell accumulating across scheduled re-runs: the per-cell store that survives a pass, driven by a slider. Repurposed 2026-08-03 from `reactive/animate.tmd`, whose `type="animate"` tick was retired — `tali.state` was its other half and is what survives | (purpose-built) |
...
| `debug/` | Algorithm debug mode (`::: {.debug}`) | a four-page exhibit for the stepped-execution widget: `sorting.tmd` is the zero-JS default (a traced `{python}` bubble sort, the automatic bars view, pointer carets on `i`/`j`, the line cursor, no author JavaScript at all); `leetcode.tmd` stacks three interview patterns on one page (binary search's three simultaneous `lo`/`mid`/`hi` carets over a shrinking range, a sliding-window string scan with a dict that earns no picture of its own, a two-pointer palindrome check where both carets land on one box); `dp.tmd` is the **grid** view, an edit-distance table filling in cell by cell with each step's read/write neighbours tinted; `custom-view.tmd` is the one page that takes the picture over, a `{js}` generator capture (`//\| trace: true`, no kernel) driving `tali.frame()` into an author-drawn view that replaces the automatic bars. Kept small on purpose (6-8 traced elements each) so no trace approaches the 5,000-frame cap | (purpose-built) |
