...
| `reactive/pyodide.tmd` | `{pyodide}` client-side Python | the cell-language registry's third language: a `{pyodide}` cell publishing a NumPy sample array with `#| name:`, a `{js}` cell consuming it across the language boundary through the same reactive graph, and a second `{pyodide}` cell showing `print` output above its returned value. Pinned server-side (wrapper emission, asset gating, single-file degradation) by `crates/core/tests/pyodide.rs`; the browser half (it actually runs and the cross-language edge fires) is Task 7's gate | (purpose-built) |
