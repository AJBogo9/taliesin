...
/// A synthetic doc with the two block shapes the live-edit moat has to cover: a raw
/// `<details>` (a single-`data-sourcepos`, stateful element) and a `:::` collapse callout
/// (a fenced div that also carries its inner blocks' `data-sourcepos`). Inserting a
/// paragraph ABOVE everything shifts every block's line numbers but not its content, so
/// both are patched in place (`SetMeta`), keeping their DOM state alive; the callout's
/// patch carries its inner positions too, so Ctrl-click inside it stays exact (see
/// `diff::nested_div_sourcepos_shift_patches_every_inner_position`). It took a full
/// `Update` from 2026-06-30 until 2026-09-24, which closed an opened callout on every
/// keystroke above it.
const SYNTHETIC: &str = "\
# Title
...
fn edit_above_preserves_a_single_sourcepos_stateful_block() {
    ...
    // The `:::` callout is a multi-`sourcepos` fenced div, and it is patched in place too.
    assert_eq!(
        m.update_count, 0,
        "a line shift re-renders nothing, the fenced callout included, got: {m:?}"
    );
}
...
fn warm_edit_payload_is_far_smaller_than_full_render() {
    ...
    // The warm-edit payload is far below a full reload: 89x on 2026-09-24, once the
    // collapse callout's line shift became a `SetMeta` carrying its inner positions. From
    // the 2026-06-30 diff hardening (`6cdbc218`) until then it was a full `Update` whose
    // html was 90% of the payload, and the ratio was 9x. The floor is 50x so a return of
    // any whole-block re-render on a line shift fails here instead of silently
    // invalidating `RESULTS.md`.
    assert!(
        m.edit_payload_bytes * 50 < m.full_html_bytes,
    ...
    assert_eq!(
        ...
        (1, 0, 0),
        "op shape changed: exactly one Insert (the typed paragraph), no Update (every \
         shifted block, the collapse callout included, is a SetMeta) and no Removes. \
