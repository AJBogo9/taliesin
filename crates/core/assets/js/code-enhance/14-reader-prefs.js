...
// is applied before paint by the pre-paint head script (taliSetTheme / taliSetReaderPref /
// taliResetReader in theme.rs), so this enhancer is only the UI. Read-only. Skipped on decks.
function taliInitReaderPrefs() {
  if (window.__qmdReaderPrefs) return;
  if (!window.taliSetReaderPref || !window.taliReaderMenu) return; // need the pre-paint API + the menu host
  ...
  function curSize() { return window.taliGetReaderPref('scale') || '1'; }
  function curWidth() { return window.taliGetReaderPref('width') || ''; }
  function curLeading() { return window.taliGetReaderPref('leading') || '1.7'; }
  function curLetter() { return window.taliGetReaderPref('letter') || '0'; }
  function curWord() { return window.taliGetReaderPref('word') || '0'; }
  ...
  var sizeSeg = seg('Text size', SIZES, curSize,
    function (v) { window.taliSetReaderPref('scale', v === '1' ? null : v); },
  ...
  var widthSeg = seg('Width', WIDTHS, curWidth,
    function (v) { window.taliSetReaderPref('width', v || null); });
  var leadingSeg = seg('Line spacing', LEADINGS, curLeading,
    function (v) { window.taliSetReaderPref('leading', v === '1.7' ? null : v); });
  var letterSeg = seg('Letter spacing', LETTERS, curLetter,
    function (v) { window.taliSetReaderPref('letter', v === '0' ? null : v); });
  var wordSeg = seg('Word spacing', WORDS, curWord,
    function (v) { window.taliSetReaderPref('word', v === '0' ? null : v); });
  ...
  reset.addEventListener('click', function () { if (window.taliResetReader) window.taliResetReader(); });
  ...
  window.taliReaderMenu.addSection('Display', body, syncAll);
}
