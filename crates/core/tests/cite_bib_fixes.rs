...
/// Slice out the single `<section class="qmd-references">…</section>` block.
fn references_section(html: &str) -> &str {
    let start = html
        .find("<section class=\"qmd-references\"")
...
fn ieee_corpus_reference_output_is_byte_stable() {
    ...
    assert_eq!(
        refs,
        "<section class=\"qmd-references\" data-block-id=\"qmd-references\">\
         <h2>References</h2>\
