...
///
/// The check is THREE-WAY as of 2026-08-10: `.github/workflows/ci.yml` was missing both of
/// those same two gates while no job there had ever run.
///
/// All three keep running them, deliberately: the hook runs locally before a push to main,
/// `gates.sh` is manual, and CI is the only one that sees a fresh clone or an outside
/// contributor's PR. So this test pins the trio rather than any one of them: a command in
/// one file and not the others is the drift it catches.
#[test]
    ...
    for needle in REQUIRED {
        ...
        assert!(
            ...
            "`.githooks/pre-push` runs `{needle}` and `.github/workflows/ci.yml` does \
             not, so an outside contributor's PR is checked for less than the author's push"
        );
        checked += 1;
    }
