...
//!
//! We emit syntect's *scope classes* (each prefixed `qhl-`) rather than inline
...
/// are easy to target (`.qhl-keyword`, `.qhl-string`, …).
const CLASS_STYLE: ClassStyle = ClassStyle::SpacedPrefixed { prefix: "qhl-" };
...
/// a `<code>` element: text is HTML-escaped and wrapped in `<span class="qhl-…">`
...
mod tests {
    ...
    fn known_language_emits_scope_classes() {
        ...
        assert!(html.contains("qhl-"), "no scope classes emitted: {html}");
        // the keyword `def` should be wrapped as a keyword scope
        assert!(html.contains("qhl-keyword"), "keyword not highlighted");
    }
