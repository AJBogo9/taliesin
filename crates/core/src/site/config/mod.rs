...
/// have. And one that loses nothing but numbers wrong: a chapter listed twice is built
/// twice, in the drawer twice under two numbers.
fn validate_chapters(
        ...
        for item in list {
            ...
            if let Some(inner) = map.get("chapters") {
                match inner.as_sequence() {
                    Some(seq) => walk(seq, warnings, src, dropped, seen),
    ...
    if let Some(chapters) = value.get("chapters") {
        match chapters.as_sequence() {
            Some(list) => walk(list, warnings, src, &dropped, &mut Default::default()),
...
mod config_tests {
    ...
    fn a_chapter_listed_twice_is_diagnosed() {
        for yaml in [
            ...
            assert!(
                w.iter().any(|m| m.message.contains("`a.tmd`")
                    && m.message.contains("more than once")
                    && m.line.is_some()),
